...
int main(int argc, char** argv) {
 //  ProfilerStart("sc1");
    ...
    leituraBarrasSimplicadoModificadaTrifasico(&grafoSDR, &numeroBarras);
    
    leituraBarrasTrifasico(&grafoSDR, &numeroBarras);
    ...
    leituraMatrizImpedanciaCorrenteTrifasicos(&Z, &maximoCorrente, numeroBarras, dadosAlimentadorSDR, numeroAlimentadores);

    int numeroAlimentadoresFicticios = 0;

    (configuracaoInicial) = alocaIndividuoModificadaTrifasico(numeroAlimentadores, numeroAlimentadoresFicticios, idConfiguracao, 1, numeroTrafos, numeroSetores);
   
    
    leituraIndividuoInicial(numeroSetores, numeroAlimentadores, configuracaoInicial);
    
    reconfiguradorHeuristicaModificada(grafoSDR, numeroBarras, dadosTrafoSDR, numeroTrafos, dadosAlimentadorSDR, numeroAlimentadores, dadosReguladorSDR, numeroReguladores, rnpSetores, numeroSetores, grafoSetores, listaChaves, numeroChaves, configuracaoInicial, maximoCorrente, Z, estadoInicial, seed);
    printf("\n\n=================PROGRAMA EXECUTADO==================\n\n");
